...
float learn_rate = 0.01;    // how much the agent weights each new sample
...
void move(int action)
{
    ...
    if (stochastic_actions)
    {
        ...
        // 2. Aplicar la probabilidad de moverse a la derecha o izquierda
        if (p >= 0.8 && p < 0.9) // 10% de probabilidad de ir a la derecha
        {
            // La acción a la derecha es (acción_original + 1) % 4
            action = (action + 1) % 4;
        }
        else if (p >= 0.9) // 10% de probabilidad de ir a la izquierda
        {
            // La acción a la izquierda es (acción_original + 3) % 4
            action = (action + 3) % 4;
        }
...
void update_q_prev_state() // Updates the Q value of the previous state
{
    ...
    {
        // SI ES TERMINAL
        Qvalues[prev_x_pos][prev_y_pos][action_taken] = Qvalues[prev_x_pos][prev_y_pos][action_taken] + learn_rate * (reward[x_pos][y_pos] - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
    }
}

void update_q_prev_state_sarsa()
{
    // Obtener el valor Q del par (Estado Siguiente, Acción Siguiente) -> Q(S', A')
    float next_q_value = Qvalues[x_pos][y_pos][action_taken2];

    // Actualizar el Q-value para el estado-acción anterior (S, A)
    // Si el estado siguiente es terminal, el valor futuro (next_q_value) es 0.
    if (((x_pos == goalx) && (y_pos == goaly)) || ((environment == 1) && (x_pos == goalx) && (y_pos == (goaly - 1))) || ((environment == 2) && (x_pos > 0) && (x_pos < goalx) && (y_pos == 0)))
    {
        Qvalues[prev_x_pos][prev_y_pos][action_taken] = Qvalues[prev_x_pos][prev_y_pos][action_taken] + learn_rate * (reward[x_pos][y_pos] - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
    }
    else // Si no es terminal, usa la fórmula completa de SARSA
    {
        Qvalues[prev_x_pos][prev_y_pos][action_taken] = Qvalues[prev_x_pos][prev_y_pos][action_taken] + learn_rate * (reward[x_pos][y_pos] + disc_factor * next_q_value - Qvalues[prev_x_pos][prev_y_pos][action_taken]);
    }
}
...
void Sarsa()
{
    // 1. Ejecuta la acción A (action_taken), que fue elegida en la iteración anterior.
    //    Llega al estado S' (nuevas coordenadas x_pos, y_pos)
    move(action_taken);

    // 2. Obtiene la recompensa R del nuevo estado S'
    cum_reward = cum_reward + reward[x_pos][y_pos];

    // 3. Elige la siguiente acción A' (action_taken2) desde el nuevo estado S'
    action_taken2 = action_selection();

    // 4. Actualiza el valor Q del estado ANTERIOR (S, A) usando la info de (R, S', A')
    update_q_prev_state_sarsa();

    // 5. La acción para la siguiente vuelta del bucle será la que acabamos de elegir
